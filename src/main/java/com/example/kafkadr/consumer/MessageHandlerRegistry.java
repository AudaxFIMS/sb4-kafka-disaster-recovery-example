...
public class MessageHandlerRegistry {
    ...
    public MessageHandlerRegistry(MessageProcessor processor,
        ...
        for (ConsumerConfig consumer : properties.getConsumers()) {
            ...
            Method method = findHandlerMethod(processor.getClass(), handlerName);
            ...
            handlers.put(topic, msg -> {
                try {
                    ...
                    method.invoke(processor, converted);
                } catch (Exception e) {
                    log.error("[{}] Handler '{}' failed: {}", topic, handlerName, e.getMessage(), e);
                }
            });
            log.info("Mapped topic '{}' -> MessageProcessor.{}(Message<{}>) [content-type={}]",
                    topic, handlerName, payloadType.getSimpleName(), contentType);
        }
    }
    ...
    private Method findHandlerMethod(Class<?> clazz, String methodName) {
        ...
        throw new IllegalStateException(
                "Handler method '%s' not found in %s. Expected: public void %s(Message<?> message)"
                        .formatted(methodName, clazz.getSimpleName(), methodName));
    }
